...
BASEDIR=$(pwd)
VMDIR="vms/${NAME}"
BASEIMG_FILENAME="${HOME}/Downloads/CentOS-7-x86_64-GenericCloud-2009.qcow2"
# CDROM=""
CDROM="-cdrom ci.iso"
...
pushd "${VMDIR}" > /dev/null
  ...
  nohup sudo qemu-system-x86_64 \
    ...
    -smp "${CPUS}" \
    -m "${MEM}G" \
    -netdev user,id=vnet0 -device virtio-net-pci,netdev=vnet0,mac=52:54:00:12:${IDX1}:${IDX2} \
    ...
    if [[ "${FIRST_RUN}" == "1" ]]; then
      ...
      while [ "${IP}" == "" ]; do
        sleep 2
        IP=$(grep "|  eth0  | True |" console.out | grep -v '::' | cut -d'|' -f4 | tr -d ' ') || true
        echo -n "."
      done
      # echo " => ${IP}"
