...
import { Grommet, Box, Card, CardFooter, CheckBox, Button, Text, TextInput, Form, FormField, 
  Footer, Anchor, TextArea, Select, CardHeader, CardBody, RadioButtonGroup } from 'grommet';
...
function App() {
  ...
  const deploy = () => 
    postData(`${srvUrl}/${provider.toLowerCase()}/deploy`, config)
  ...
  const destroy = () =>
    postData(`${srvUrl}/${provider.toLowerCase()}/destroy`, config)
    .then(response => response.body)
    .then(rb => processResponse(rb))
    .then(stream => new Response(stream, { headers: { 'Content-Type': 'text/html' } }).text())
          ...
          <Form
            ...
            >
              { Object.keys(config).filter(k => !k.includes('is_') && !k.includes('region')).map( key => 
                  <FormField name={key} htmlfor={key} label={ key.replace('is_', '').toUpperCase() } key={key} required={ !key.includes('is_') } margin='small'>
                ...
                <CardFooter direction='column'>
                    ...
                    {
                      Object.keys(usersettings).filter(k => !k.includes('is_')).map( key => 
                        <FormField name={key} htmlfor={key} label={ key.replace('is_', '').toUpperCase() } key={key} required={ !key.includes('is_') } margin='small'>
                  ...
                  <Box direction='row' justify='center'>
                    ...
                    { config['is_ha'] !== undefined  && <CheckBox toggle reverse label='Enable HA (Runtime only)' checked={ config['is_ha'] } onChange={ () => setConfig( old => ( {...old, 'is_ha': !old['is_ha'] }) ) } /> }
